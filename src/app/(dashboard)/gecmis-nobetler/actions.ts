...
async function analyzeUploadedFile(
  file: File
): Promise<{ analysis: ImportAnalysis; fileName: string }> {
  const buffer = Buffer.from(await file.arrayBuffer());
  const inputRows = parseHistoricalExcel(buffer);
  ...
  const analysis = analyzeImportRows(inputRows, {
    pharmacies,
    regions,
...
export async function historicalImportAction(
  ...
  let fileName: string;
  try {
    ({ analysis, fileName } = await analyzeUploadedFile(file));
  } catch (error) {
  ...
  if (mode !== "import") {
    ...
    return toPreviewState(analysis, fileName, message, analysis.canImport);
  }

  // İçe aktarma modu: sunucu tarafında yeniden doğrula, hata varsa reddet.
