...
const ALLOWLIST: Record<string, string> = {
  ...
  "src/app/platform/kurumlar/actions.ts:97": "[platform-only operation] pre-check for the new first-ADMIN's email against the globally-unique User.email constraint, guarded by requirePlatformAdmin(); intentionally organization-agnostic since email uniqueness is global, not per-tenant.",
